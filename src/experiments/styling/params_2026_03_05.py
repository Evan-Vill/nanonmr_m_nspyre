...
class FitParamsWidget(QtWidgets.QWidget):
    def __init__(self, params_config: Dict, get_param_value_funs: Optional[Dict] = None):
        super().__init__()
        self.params_config = params_config
        self.widgets = {}
        self.fit_labels = {}
    ...
    def all_params(self):
        return {p: getattr(self, p) for p in self.params_config}
    ...
    def __getattr__(self, attr: str):
        if attr in self.params_config:
            ...
            try:
                ...
                raise ValueError(
                    f'Parameter [{attr}] has no function for retrieving its value '
                    'from the GUI. Set it using get_param_value_funs.'
                ) from err
            return fun(widget)
        return self.__getattribute__(attr)
