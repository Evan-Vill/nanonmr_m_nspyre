    ...
    def CASR_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        try:
            ...
            try:
                ...
                times = np.linspace(t_seq - wait_time - laser_read_time / 2, cfg.num_pts * t_seq, cfg.num_pts) * 1e-9 # FIXME: this is a bit hacky - need to make sure time points correspond to center of readout pulse for accurate fitting, but currently hardcoded based on current sequence structure (laser readout pulse at end of sequence with fixed duration)
