...
class SpinMeasurements:
    ...
    def get_plot_metadata(self, exp_type: str) -> dict:
        ...
        metadata = {
            ...
            "Nitrogen T1": {
                ...
                "xlabel": "Delay (µs)",
                "ylabel": "Signal (V) or Norm. Signal",
                "pl_title": "Nitrogen T1 PL Time Trace",
                "pl_xlabel": "Readout Window (ns)",
                "pl_ylabel": "Signal (V)",
            },
            "QLE XY8": {
                ...
                "xlabel": "Free Precession Interval (µs)",
                "ylabel": "Signal (V) or Norm. Signal",
                "pl_title": "QLE XY8 PL Time Trace",
                "pl_xlabel": "Readout Window (ns)",
                "pl_ylabel": "Signal (V)",
            },
            "QLE Corr Spec": {
                "title": "QLE Correlation Spectroscopy",
                "xlabel": "Correlation Time (µs)",
                "ylabel": "Signal (V) or Norm. Signal",
                "pl_title": "QLE Correlation Spectroscopy PL Time Trace",
                "pl_xlabel": "Readout Window (ns)",
                "pl_ylabel": "Signal (V)",
            },
            "Cnot NE time": {
                "title": "CNOT NE Time",
                "xlabel": "CNOT Duration (µs)",
                "ylabel": "Signal (V) or Norm. Signal",
                "pl_title": "CNOT NE Time PL Trace",
                "pl_xlabel": "Readout Window (ns)",
                "pl_ylabel": "Signal (V)",
            },
        }
    ...
    @managed_experiment(token_prefix="POWERCALIBRATION", dataset_key="dataset")
    def power_calibration_scan(self, *, mgr, data, pl_data, token, **kwargs):
        cfg = nvcfg.PowerCalibrationScanCfg(**kwargs)
        #setup
        laser_shutter = mgr.laser_shutter
        daq = mgr.daq
        sig_gen = mgr.sg
        ...
        if powers.size < 1:
            raise ValueError("power_num_pts must be at least 1")

        mw_times = np.linspace(
            cfg.start,
            cfg.stop,
            cfg.num_pts,
            dtype=float,
        )
        pts_per_power = len(mw_times)
        ...
        self._configure_sig_gen_iq(
            sig_gen,
            carrier_freq=sig_gen_freq,
            rf_power=cfg.rf_power,
        )

        dig_cfg = self.digitizer_configure(
            exp_type="Rabi",
            num_pts=pts_per_power,
            iters=1,                      # acquire one Rabi curve at a time
            segment_size=cfg.segment_size,
            sampling_freq=cfg.dig_sampling_freq,
            dig_amplitude=cfg.dig_amplitude,
            readout_channel=int(cfg.read_channel),
            both_channels=cfg.both_channels,
            coupling=cfg.dig_coupling,
            termination=cfg.dig_termination,
            pretrig_size=cfg.pretrig_size,
        ...
        #experiment loop
        power_cal_sweeps = StreamingList()
        raw_rabi_data= []
        exp_start_time = time.perf_counter()
        ...
        for i in range(cfg.iters):
            ...
            pi_times = np.full(len(powers), np.nan, dtype=float)
            ...
            for power_index, power in enumerate(powers):
                ...
                # Build seq for one Rabi scan
                sequence = ps.Power_Calibration(
                    cfg.laser_init * 1e9,
                    mw_times * 1e9,
                ...
                # AWG sequence for this MW power
                hdawg.set_sequence(
                    seq="Power Calibration",
                    i_offset=cfg.i_offset,
                    q_offset=cfg.q_offset,
                    sideband_power=power,
                    sideband_freq=cfg.sideband_freq,
                    iq_phases=iq_phases,
                    pi_pulses=mw_times,
                    num_pts=cfg.num_pts,
                ...
                self.dig.assign_param(dig_cfg)

                ps.set_soft_trigger()
                ps.stream(
                    sequence,
                    PulseStreamer.REPEAT_INFINITELY,
                    owner=token,
                )
                ...
                # Extract data from this Rabi curve- not calling acquire data function yet
                result_raw = self.dig.acquire()

                if result_raw is None:
                    _logger.warning(
                        "No digitizer data returned for power %.3f dBm",
                        power,
                    )
                    iteration_rabi_data.append({
                        "power": power,
                        "x": mw_times.copy(),
                        "signal": None,
                        "background": None,
                ...
                # Parse only the Rabi curve that was just acquired.
                parsed = self.parse_analog_math(
                    result_raw,
                    "Rabi",
                    pts_per_power,
                )
                ...
                if cfg.both_channels:
                    ...
                    signal = np.asarray(ch0[0], dtype=float)
                    background = np.asarray(ch0[1], dtype=float)

                    signal_ch2 = np.asarray(ch1[0], dtype=float)
                    background_ch2 = np.asarray(ch1[1], dtype=float)

                else:
                    signal, background = parsed
                ...
                if cfg.both_channels:
                    rabi_result["signal_ch2"] = signal_ch2.copy()
                    rabi_result["background_ch2"] = background_ch2.copy()
                ...
                with warnings.catch_warnings():
                    ...
                    try:
                        ...
                        # First fit parameter is the π-pulse time
                        pi_time = float(fit_value[0])

                        pi_times[power_index] = pi_time

                        rabi_result["pi_time"] = pi_time
                        rabi_result["fit_value"] = fit_value
                        rabi_result["fit_error"] = fit_error
                        rabi_result["fit_x"] = fit_x
                        rabi_result["fit_y"] = fit_y
                        rabi_result["fit_units"] = fit_units
                    except:
                        pass
                     
            raw_rabi_data.append(iteration_rabi_data)
            calibration_curve = np.stack([
                        powers,
                        pi_times,
                    ])
            
            power_cal_sweeps.append(calibration_curve)
            power_cal_sweeps.updated_item(-1)
            ...
            data.push({
                        ...
                        "xlabel": "MW Power (dBm)",
                        "ylabel": "Pi Pulse Time (ns)",
                        "datasets": {
                            "power_calibration": power_cal_sweeps,
                        },
                    })
        ...
        self.queue_from_exp.put_nowait(
    ...
    @managed_experiment(token_prefix="NITROGENT1", dataset_key="dataset")
    def Nitrogen_T1_scan(self, *, mgr, data, pl_data, token, **kwargs):
        cfg = nvcfg.NitrogenT1ScanCfg(**kwargs)
        ...
        t_op_times = np.linspace(cfg.start, cfg.stop, cfg.num_pts)
        sig_gen_freq, iq_phases = self.choose_sideband(
            cfg.sideband,
            cfg.freq,
            cfg.sideband_freq,
        ...
        sequence = ps.Nitrogen_T1(
            params= t_op_times * 1e9,
            read_time= cfg.laser_readout * 1e9,
            cnot_ne=cfg.cnot_ne * 1e9,
            laser_time=cfg.laser_init * 1e9,
        )
        ...
        hdawg.set_sequence(
            ...
            sideband_freq=cfg.sideband_freq,
            iq_phases=iq_phases,
            pi_pulse=cfg.cnot_en,
            cnot_en=cfg.cnot_en,
            cnot_ne=cfg.cnot_ne,
            rf_power=cfg.rf_power,
            rf_freq=cfg.rf_freq,
            rf_phase=cfg.rf_phase,
            num_pts=cfg.num_pts,
        )

        fit_value, fit_error, fit_units = [], [], []
        fit_x = t_op_times
        fit_y = np.ones(cfg.num_pts)

        dig_cfg = self.digitizer_configure(
            exp_type="Nitrogen T1",
            num_pts=cfg.num_pts,
            iters=cfg.iters,
            segment_size=cfg.segment_size,
            sampling_freq=cfg.dig_sampling_freq,
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector):
            ...
            for i in range(cfg.iters):
                if experiment_widget_process_queue(self.queue_to_exp) == "stop":
                    stopped = True
                    iters_completed = i
                    percent_completed = int(100 * i / cfg.iters)
                    break
    ...
    @managed_experiment(token_prefix="QLEXY8", dataset_key="dataset")
    def qle_xy8_scan(self, *, mgr, data, pl_data, token, **kwargs):
        cfg = nvcfg.QLEXY8ScanCfg(**kwargs)
    ...
    @managed_experiment(token_prefix="QLECORRSPEC", dataset_key="dataset")
    def qle_corr_spec_scan(self, *, mgr, data, pl_data, token, **kwargs):
        cfg = nvcfg.QLECorrSpecScanCfg(**kwargs)
    ...
    @managed_experiment(token_prefix="CNOTNE", dataset_key="dataset")
    def cnot_ne_time_scan(self, *, mgr, data, pl_data, token, **kwargs):
        cfg = nvcfg.CnotNETimesCfg(**kwargs)
        ...
        rf_times = np.linspace(
            cfg.rf_time_start,
            cfg.rf_time_stop,
            cfg.rf_time_num_pts,
        )

        sequence = ps.Cnot_NE_time(
            params=rf_times * 1e9,
            read_time=cfg.laser_readout * 1e9,
            laser_time=cfg.laser_init * 1e9,
        )
        ...
        dig_cfg = self.digitizer_configure(
            exp_type="Cnot NE time",
            num_pts=cfg.rf_time_num_pts,
            iters=cfg.iters,
            segment_size=cfg.segment_size,
            sampling_freq=cfg.dig_sampling_freq,
            dig_amplitude=cfg.dig_amplitude,
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector):
            ...
            for i in range(cfg.iters):
                self.acquire_data(
                    ...
                    data=data,
                    fit_x=rf_times * 1e6,
                    fit_y=np.ones(cfg.rf_time_num_pts),
                    fit_value=[],
                    fit_error=[],
                    subseq_sweeps=subseq_sweeps,
                    subseq_2_sweeps=subseq_2_sweeps,
                ...
                self.queue_from_exp.put_nowait(
                    self.build_status_msg(
                        status="in progress",
                        percent_completed=int(100 * (i + 1) / cfg.iters),
                        fit_value=None,
                        fit_error=None,
