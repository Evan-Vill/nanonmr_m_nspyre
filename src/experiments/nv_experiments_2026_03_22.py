...
class SpinMeasurements:
    ...
    def parse_analog_math(self, array, exp_type, pts):
        ...
        for ch in channels:
            ...
            ch_out = []
            for j in range(n):
                avg = per_point[:, j].reshape(runs, pts).mean(axis=0)
                ch_out.append(avg)
    ...
    def odmr_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):     
            ...
            for i in range(cfg.iters):
                ...
                try:
                    sig, bg = self.parse_analog_math(odmr_result, "CW ODMR", cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def rabi_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    result_raw = self.dig.acquire() # acquire data from digitizer
                except Exception as e:
                    failed = True
                    exception_type = type(e).__name__
                    iters_completed = i
                ...
                try:
                    parsed = self.parse_analog_math(result_raw, "Rabi", cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
                    
                if cfg.both_channels:
                    (sig_ch0, bg_ch0), (sig_ch1, bg_ch1) = parsed
                    signal_sweeps.append(np.stack([mw_times, sig_ch0])); signal_sweeps.updated_item(-1)
                    background_sweeps.append(np.stack([mw_times, bg_ch0])); background_sweeps.updated_item(-1)
                    signal_2_sweeps.append(np.stack([mw_times, sig_ch1])); signal_sweeps.updated_item(-1)
                    background_2_sweeps.append(np.stack([mw_times, bg_ch1])); background_sweeps.updated_item(-1)
                    data.push({
                        "params": {
                            "kwargs": kwargs, 
                            "iters_completed": iters_completed, 
                            "elapsed": time.perf_counter() - exp_start_time
                        },
                        ...
                        "datasets": {
                            ...
                            "background": background_sweeps, 
                            "signal_2": signal_2_sweeps,
                            "background_2": background_2_sweeps,
                            "x_fit": fit_x, 
                            "y_fit": fit_y, 
                            "fit_value": fit_value, 
                            "fit_error": fit_error
                ...
                if pl_data is not None:
                    sig_pl, bg_pl = self.analog_pl_math(result_raw, "Rabi", cfg.pl_pt, cfg.num_pts)
    ...
    def pulsed_odmr_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    sig, bg = self.parse_analog_math(pulsed_odmr_result, "Pulsed ODMR", cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def pulsed_odmr_rf_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    rf_sig, rf_bg, no_rf_sig, no_rf_bg = self.parse_analog_math(pulsed_odmr_result, 'DEER', cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def DEER_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    dark_sig, dark_bg, echo_sig, echo_bg = self.parse_analog_math(deer_result, "DEER", cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def DEER_FID_CD_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    dark_sig, dark_bg, echo_sig, echo_bg, cd_sig, cd_bg = self.parse_analog_math(cd_result, 'CD', cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def DEER_corr_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    sig, bg = self.parse_analog_math(corr_result, 'Corr', cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def DEER_T1_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    with_py, without_py, with_ny, without_ny = self.parse_analog_math(corr_result, 'DEER', cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
    ...
    def DEER_T2_scan(self, *, mgr, data, pl_data, token, **kwargs):
        ...
        with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
            ...
            for i in range(cfg.iters):
                ...
                try:
                    sig, bg = self.parse_analog_math(deer_t2_result, 'DEER T2', cfg.num_pts) # partition buffer into signal and background datasets
                except ValueError:
                    continue
            ...
            try:
                ...
                with self._shutter_open(laser_shutter, daq, cfg.detector), _rf_on(sig_gen):
                    ...
                    for i in range(cfg.iters):
                        ...
                        try:
                            sig, bg = self.parse_analog_math(casr_result, "CASR", cfg.num_pts) # partition buffer into signal and background datasets
                        except ValueError:
                            continue
