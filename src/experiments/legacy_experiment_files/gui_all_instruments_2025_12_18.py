...
try:
    ...
    def _alive(w):
        # """Fallback: always assume valid if check unavailable."""
...
class InstWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.updateTimer = QTimer(self) #create a timer that will try to update that widget with messages from the from_exp_queue
        ...
        self.hwTimer = QTimer(self) # timer to periodically check hardware status
    ...
    # generic safe caller with reconnect + retry once
    # def _call(self, fn):
    #     try:
    #         return fn()
    #     except Exception:
    ...
    def _call(self, fn, *, context=""):
        ...
        try:
            ...
            try:
                ...
                # optional: keep last error per context to avoid repeat prints
    ...
    #         except Exception:
    #             pass
    #         # if you keep a persistent InstrumentManager:
    #         try:
    #             self._mgr.__exit__(None, None, None)
    #         except Exception:
    #             pass
    ...
    def init_sg396_widgets(self):
        ...
        self.sg396_params_widget_2 = ParamsWidget(
            {
                'sideband_freq': {'display_text': 'Sideband Mod. Freq.: ',
    ...
    def init_laser_widgets(self):
        ...
        self.optics_flipper_label = QLabel("Microscope Configuration:")
        self.optics_flipper_label.setFixedHeight(40)

        self.optics_flipper_b1 = QRadioButton("Epifluorescence")
        self.optics_flipper_b1.toggled.connect(lambda:self.toggle_optics_mode(self.optics_flipper_b1))
        self.optics_flipper_b1.setStyleSheet(laser_style)

        self.optics_flipper_b2 = QRadioButton("TIR")
        self.optics_flipper_b2.toggled.connect(lambda:self.toggle_optics_mode(self.optics_flipper_b2))
        self.optics_flipper_b2.setStyleSheet(laser_style)

        # exclusivity logic for optics flipper buttons
        ...
        self.laser_current_label = QLabel("Set Diode Current (%):")
        ...
        self.laser_power_set_button = QPushButton("Set")
        self.laser_power_set_button.setStyleSheet(laser_button_style)
        self.laser_power_set_button.clicked.connect(lambda: self.laser_power_changed())

        self.laser_current_setpt_label = QLabel("Current Setpoint: 0%") # TODO: add to layout
        self.laser_current_setpt_label.setFont(QFont("Sanserif", 15))
        self.laser_power_label = QLabel("Power Setpoint: 0 mW")
        self.laser_power_label.setFont(QFont("Sanserif", 15))
            
        self.laser_baseplate_temp_label = QLabel("Baseplate Temp: ---")
        self.laser_baseplate_temp_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
        self.laser_baseplate_temp_label.setFixedHeight(40)
        self.laser_diode_temp_label = QLabel("Diode Temp: ---")
        self.laser_diode_temp_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
        self.laser_diode_temp_label.setFixedHeight(40)

        # predefine initial labels
        ...
        self.laser_interlock_status_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
        self.laser_interlock_status_label.setFixedHeight(40)

        self.laser_emit_status_label = QLabel("Emission status")
        self.laser_emit_status_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
        ...
        self.laser_shutter_status_label = QLabel("Shutter status")
        self.laser_shutter_status_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
        self.laser_shutter_status_label.setFixedHeight(40)
        self.laser_shutter_status_label.setFixedWidth(175)

        self.laser_alarm_reset_button = QPushButton("Reset Alarm")
        self.laser_alarm_reset_button.setStyleSheet(laser_button_style)
        self.laser_alarm_reset_button.clicked.connect(lambda: self.laser_alarm_reset_clicked())

        self.ps_status_label = QLabel("Pulse Streamer: CONSTANT")
        self.ps_status_label.setStyleSheet("color: white; background-color: #222; border: 2px solid #444;")
    ...
    def init_flipper_widgets(self):
        ...
        self.flipper_b1.toggled.connect(lambda:self.toggle_detector_mode(self.flipper_b1))
        self.flipper_b1.setStyleSheet(flipper_style)

        self.flipper_b2 = QRadioButton("BPD")
        self.flipper_b2.toggled.connect(lambda:self.toggle_detector_mode(self.flipper_b2))
    ...
    def layouts(self):
        ...
        self.laser_layout.addWidget(self.laser_label,1,1,1,3)

        self.laser_layout.addWidget(self.optics_flipper_label,2,1,1,1)
        self.laser_layout.addWidget(self.optics_flipper_b1,2,2,1,1)
        self.laser_layout.addWidget(self.optics_flipper_b2,2,3,1,1)

        self.laser_layout.addWidget(self.laser_current_label,3,1,1,1)
        self.laser_layout.addWidget(self.laser_current_value,3,2,1,1)
        self.laser_layout.addWidget(self.laser_power_set_button,3,3,1,1)

        self.laser_layout.addWidget(self.laser_current_setpt_label,4,1,1,2)
        self.laser_layout.addWidget(self.laser_power_label,4,3,1,1)

        self.laser_layout.addWidget(self.laser_baseplate_temp_label,5,1,1,2)
        self.laser_layout.addWidget(self.laser_diode_temp_label,5,3,1,1)

        self.laser_layout.addWidget(self.laser_status_label,6,1,1,1)
        self.laser_layout.addWidget(self.laser_interlock_status_label,6,2,1,1)
        self.laser_layout.addWidget(self.laser_alarm_reset_button,6,3,1,1)

        self.laser_layout.addWidget(self.laser_b1,7,1,1,1)
        self.laser_layout.addWidget(self.laser_b2,7,2,1,1)
        self.laser_layout.addWidget(self.laser_emit_status_label,7,3,1,1)

        self.laser_layout.addWidget(self.laser_shutter_button,8,1,1,1)
        self.laser_layout.addWidget(self.laser_shutter_status_label,8,2,1,2)

        self.laser_layout.addWidget(self.ps_status_label,9,1,1,3)
    ...
    #     try:
    #         st = self.call_ps(lambda ps: ps.get_status())  # returns pulse streamer status object
    #             # st.owner: str | None, st.streaming: bool, st.mode: "SEQUENCE"/"CONSTANT"/"IDLE"
    #         owner = st.get("owner")
    #         mode = st.get("mode", "IDLE")
    #         streaming = bool(st.get("streaming", False))

    #         # Color / disable logic
    #         if owner and not str(owner).startswith("GUI_"):
    #             # someone else (experiment) owns it -> lock out CW toggles
    #             self._set_cw_controls_enabled(False)
    #             self.ps_status_label.setStyleSheet("color: black; background-color: gold; border: 2px solid black;")
    #             txt = f"Pulse Streamer: {st.mode} (owner: {st.owner})"
    #         else:
    #             # free or GUI-owned -> allow user control
    #             self._set_cw_controls_enabled(True)
    #             self.ps_status_label.setStyleSheet("color: white; background-color: #222; border: 2px solid #444;")
    #             txt = f"Pulse Streamer: {st.mode}"

    #         self.ps_status_label.setText(txt)
    #         self._ps_last_error = None

        # except Exception as e:
        #     # stash the last error so we only show it once
        #     msg = f"{type(e).__name__}: {e}"
        #     if getattr(self, "_ps_last_error", None) != msg:
        #         self._ps_last_error = msg
        #         print(f"[PS status] {msg}")  # or logger.warning(...)

        #     self._set_cw_controls_enabled(False)
        #     self.ps_status_label.setStyleSheet(
        #         "color: white; background-color: #800; border: 2px solid #f00;"
        #     )
        #     self.ps_status_label.setText("Pulse Streamer: status error")
    ...
    def check_laser_temp_status(self):
        ...
        try:
            ...
            if base_temp is None or diode_temp is None:
                ...
                self._set_label_if_changed(base_lbl,  "Baseplate Temp: ---", "color: #bbb;")
            ...
            self._set_label_if_changed(
                base_lbl, "Baseplate Temp: ERROR", "color: white; background-color: #800; border: 2px solid #f00;")
            
            self._set_label_if_changed(
                diode_lbl, "Diode Temp: ERROR", "color: white; background-color: #800; border: 2px solid #f00;")
    ...
    def check_laser_shutter_status(self):
        ...
        try:
            state = self.call_laser_shutter(lambda laser_shutter: laser_shutter.get_shutter_state())
            if state is None:
                self._set_label_if_changed(lbl, "Shutter: ---", "color: #bbb;")
                return
    ...
    def check_ps_status(self):
        ...
        try:
            st = self.call_ps(lambda ps: ps.get_status())   # {"owner","mode","streaming"}
            if st is None:
                self._set_label_if_changed(ps_lbl, "Pulse Streamer: ---", "color: #bbb;")
                return
            owner = st.get("owner")
            mode  = st.get("mode", "IDLE")

            # enable/disable CW controls if ownership is external
            external_owner = bool(owner and not str(owner).startswith(self.GUI_OWNER_PREFIX))
            ...
            if external_owner:
                ...
                if cw_on:
                    ...
                    if _alive(pow_lbl):
                        self._set_label_if_changed(pow_lbl, pow_lbl.text(), "color: limegreen;")
                else:
                    if _alive(emit_lbl):
                        self._set_label_if_changed(
                            emit_lbl,
    ...
    # def check_laser_shutter_status(self):
    #     try:
    #         shutter_state = self.call_laser_shutter(lambda laser_shutter: laser_shutter.get_shutter_state())
    #         if shutter_state == "Active":
    #             self.laser_shutter_status_label.setStyleSheet("color: black; background-color: white; border: 4px solid black;")
    #             self.laser_shutter_status_label.setText("Shutter: OPEN")
    #         else:
    #             self.laser_shutter_status_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
    #             self.laser_shutter_status_label.setText("Shutter: CLOSED")

    #     except Exception as e:
    #         if _alive(getattr(self, "laser_shutter_status_label", None)):
    #             self.laser_shutter_status_label.setStyleSheet("color: white; background-color: #800; border: 2px solid #f00;")
    #             self.laser_shutter_status_label.setText("Shutter: ERROR")
    #         msg = f"{type(e).__name__}: {e}"
    #         print(f"[Laser Shutter status] {msg}")

    # def check_ps_status(self):
    #     try:
    #         st = self.call_ps(lambda ps: ps.get_status())
    #         owner = st.get("owner")
    #         mode  = st.get("mode", "IDLE")

    #         # enable/disable
    #         self._set_cw_controls_enabled(not (owner and not str(owner).startswith("GUI_")))

    #         # update label if it still exists
    #         if _alive(getattr(self, "ps_status_label", None)):
    #             if owner and not str(owner).startswith("GUI_"):
    #                 self.ps_status_label.setStyleSheet("color: black; background-color: gold; border: 2px solid black;")
    #                 self.ps_status_label.setText(f"Pulse Streamer: {mode} (owner: {owner})")
    #                 # update laser emission status
    #                 # self.laser_emit_status_label.setText("Emission: ON")
    #                 # self.laser_emit_status_label.setStyleSheet("color: black; background-color: limegreen; border: 4px solid black;")
    #                 # self.laser_power_label.setStyleSheet("color: limegreen;")

    #             else:
    #                 self.ps_status_label.setStyleSheet("color: white; background-color: #222; border: 2px solid #444;")
    #                 self.ps_status_label.setText(f"Pulse Streamer: {mode}")

    #         self._ps_last_error = None

    #     except Exception as e:
    #         # don’t touch dead widgets
    #         self._set_cw_controls_enabled(False)
    #         if _alive(getattr(self, "ps_status_label", None)):
    #             self.ps_status_label.setStyleSheet("color: white; background-color: #800; border: 2px solid #f00;")
    #             self.ps_status_label.setText("Pulse Streamer: STATUS ERROR")
    ...
    def sg396_emit_button_clicked(self):
        ...
        self.call_awg(lambda awg: awg.set_sequence(**{'seq': 'Test',
    ...
    def laser_temp_clicked(self, component):
        match component:
            ...
            case 'base':
                heat_sink_temp = self.call_laser(lambda laser: laser.get_baseplate_temp())
                self.laser_baseplate_temp_label.setStyleSheet("color: white; background-color: black; border: 4px solid black;")
    ...
    def get_laser_status(self):
        ...
        if laser_status in ("Warm Up", "Standby", "Searching SLM point"):
            ...
            style = "color: black; background-color: limegreen; border: 4px solid black;"
    ...
    # def get_laser_status(self):
    #     laser_status = self.call_laser(lambda laser: laser.get_status())
    #     if laser_status == 'Warm Up' or laser_status == 'Standby' or laser_status == 'Searching SLM point':
    #         self.laser_status_label.setStyleSheet("color: black; background-color: orange; border: 4px solid black;")
    #         self.laser_status_label.setText(f"{laser_status}")
    #     elif laser_status == 'Laser ON':
    #         self.laser_status_label.setStyleSheet("color: black; background-color: limegreen; border: 4px solid black;")
    #         self.laser_status_label.setText(f"{laser_status}")
    #     elif laser_status == 'Error' or laser_status == 'Alarm':
    #         self.laser_status_label.setStyleSheet("color: black; background-color: red; border: 4px solid black;")
    #         self.laser_status_label.setText(f"{laser_status}")
    #     else:
    ...
    def get_interlock_status(self):
        ...
        if interlock_status == 1:
            self.laser_interlock_status_label.setStyleSheet("color: black; background-color: limegreen; border: 4px solid black;")
            self.laser_interlock_status_label.setText(f"Interlock: CLOSED")
        else:
            self.laser_interlock_status_label.setStyleSheet("color: black; background-color: red; border: 4px solid black;")
            self.laser_interlock_status_label.setText(f"Interlock: OPEN")
        ...
        match b.text():
            case 'CW ON':
                if b.isChecked() == True:
                    ...
                    self.laser_emit_status_label.setStyleSheet("color: black; background-color: white; border: 4px solid black;")
    ...
    def _set_cw_controls_enabled(self, enabled: bool):
        ...
        # # status label(s) — same idea
        # lbl = getattr(self, "ps_status_label", None)
        # if _alive(lbl):
        #     # you can set stylesheet/text elsewhere; here we just ensure not to crash
        #     pass

        # # any other labels you tweak in this function:
        # emit_lbl = getattr(self, "laser_emit_status_label", None)
        # if _alive(emit_lbl):
        #     # safe to style if needed
        #     pass
    ...
    def toggle_optics_mode(self, b):
        ...
        match b.text():                 
            case 'TIR':
                self.call_daq(lambda daq: daq.write_do_task('flip mirror', detector='tirf'))
            case _:
                self.call_daq(lambda daq: daq.write_do_task('flip mirror', detector='apd'))
    ...
    def toggle_detector_mode(self, b):
        ...
        match b.text():
            case 'APD':
                self.call_daq(lambda daq: daq.write_do_task('flip mirror 2', detector='apd'))
            
            case 'BPD':
                self.call_daq(lambda daq: daq.write_do_task('flip mirror 2', detector='bpd'))
    ...
    def laser_power_changed(self):
        try:
            ...
            self.laser_power_label.setText(f"Power Setpoint: {str(round((self.laser_power),1))} mW")
            self.call_laser(lambda laser: laser.set_diode_current_realtime(self.laser_power))
