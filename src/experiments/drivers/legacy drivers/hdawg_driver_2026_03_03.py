...
{self.create_pulses(2, 2, "waveRF_pihalf")} waitWave();}}"""]
    ...
    def compile_sequence(self, group, program):
        ...
        self.daq.sync()
        # self.daq.syncSetInt(f"/{self.device:s}/awgs/1/enable", True)
        print("AWG0 enable:", self.daq.getInt(f"/{self.device}/awgs/0/enable"),
            "ready:", self.daq.getInt(f"/{self.device}/awgs/0/ready"))
        print("AWG1 enable:", self.daq.getInt(f"/{self.device}/awgs/1/enable"),
            "ready:", self.daq.getInt(f"/{self.device}/awgs/1/ready"))
