...
class HDAWG:
    ...
    def set_sequence(self, **kwargs):
        ...
        match kwargs['seq']:
            ...
            case 'Power Calibration':

                rabi_pulses = kwargs['pi_pulses']

                # Single microwave power
                power = kwargs['sideband_power']
                amplitude = self.convert_mw_power(power)
                ...
                for i in range(kwargs['num_pts']):
                    ...
                    i_waves.append(
                        self.create_rect_wave(
                            wave_name_i,
                            rabi_pulses[i] * self.sampling_rate[0],
                            amplitude
                        )
                    )
                ...
                self.daq.setDouble(
                    f"/{self.device}/oscs/0/freq",
                    kwargs['sideband_freq']
                )
            ...
            case "Cnot NE time":
                rf_times = np.asarray(kwargs["rf_times"], dtype=float)

                if rf_times.ndim != 1 or rf_times.size == 0:
                    raise ValueError("rf_times must be a non-empty 1-D array")

                num_pts = rf_times.size
                cnot_en = float(kwargs["cnot_en"])
                #TODO: use cnot en specific power from power calibration
                sideband_power = self.convert_mw_power(
                    kwargs["sideband_power"]
                )
                rf_power = self.convert_mw_power_rf(
                    kwargs["rf_power"]
                )
                ...
                #TODO: update phase kwarg reference from experiment file
                i_phase = [
                ...
                for index, rf_time in enumerate(rf_times):
                    if rf_time < 0:
                        raise ValueError("RF durations must be non-negative")

                    rf_wave_name = f"waveRF_cnot_ne_{index}"

                    rf_waves.append(
                        self.create_rect_wave(
                            rf_wave_name,
                            rf_time * self.sampling_rate[1],
                            rf_power,
                        )
                    )

                    # IQ group timing is expressed in IQ-group samples.
                    iq_steps.append(
                        f"""
                        // Point {index}
                        // TODO: update phase kwarg reference from experiment file
                        {self.set_pulse_phases("x", kwargs["iq_phases"])}
                        {self.create_iq_pulses(
                            1, 1, "I_cnot_en", 2, "Q_cnot_en"
                        )}
                        waitWave();
            ...
            case 'Nitrogen T1':
                # MW CNOT_en
                i_wave_cnot_en = [self.create_rect_wave("I_cnot_en",kwargs['cnot_en'] * self.sampling_rate[0],self.convert_mw_power(kwargs['sideband_power']))]

                q_wave_cnot_en = [self.create_rect_wave("Q_cnot_en",kwargs['cnot_en'] * self.sampling_rate[0],self.convert_mw_power(kwargs['sideband_power']))]

                #TODO: define cnot en phases

                # RF
                awg_wave_rf = [self.create_rect_wave(
                    "waveRF_cnot_ne",
                    kwargs['cnot_ne'] * self.sampling_rate[1],
                    self.convert_mw_power_rf(kwargs['rf_power'])
                )]

                awg_pulses: List[str] = []

                awg_pulses.append(f"""
            repeat({kwargs['num_pts']}){{

                // FIRST MW CNOT_en
                {self.set_pulse_phases('x', kwargs['iq_phases'])}
                {self.create_iq_pulses(1,1,"I_cnot_en",2,"Q_cnot_en")}
                waitWave();
            ...
            repeat({kwargs['num_pts']}){{
                ...
                {self.set_sine_phase(1, kwargs['rf_phase'])}
                {self.create_pulses(2, 2, "waveRF_cnot_ne")}
                waitWave();
            ...
            case 'QLE Corr Spec':
                ...
                i_wave_cnot_en = [self.create_rect_wave("I_cnot_en",kwargs['cnot_en'] * self.sampling_rate[0],self.convert_mw_power(kwargs['sideband_power']))] #TODO: modify these to correct power levels from pwoer calibration
                q_wave_cnot_en = [self.create_rect_wave("Q_cnot_en",kwargs['cnot_en'] * self.sampling_rate[0],self.convert_mw_power(kwargs['sideband_power']))]
                ...
                read_cycle_time = kwargs['cnot_en'] + kwargs['read_time']
                max_reads = int(kwargs['total_time'] / read_cycle_time)
            ...
            repeat({kwargs['num_pts']}){{
                ...
                {self.set_pulse_phases('x', kwargs['iq_phases'])}
                {self.create_iq_pulses(1,1,"I_pihalf_x",2,"Q_pihalf_x")}
                waitWave();

                repeat({kwargs['n']}){{

                    {self.set_pulse_phases('x', kwargs['iq_phases'])}
                    {self.create_iq_pulses(1,1,"I_pi_x",2,"Q_pi_x")}
                    waitWave();

                    {self.set_pulse_phases('y', kwargs['iq_phases'])}
                    {self.create_iq_pulses(1,1,"I_pi_y",2,"Q_pi_y")}
                    waitWave();
                ...
                {self.set_pulse_phases('y', kwargs['iq_phases'])}
                {self.create_iq_pulses(1,1,"I_pihalf_y",2,"Q_pihalf_y")}
                waitWave();
                ...
                // WAIT DURING RF PULSE

                playZero({int(kwargs['cnot_ne'] * self.sampling_rate[0])});
                waitWave();
            ...
            case "QLE XY8":
                num_pts = int(kwargs["num_pts"])
                n = int(kwargs["n"])

                if num_pts < 1 or n < 1:
                    raise ValueError("num_pts and n must be at least 1")
                ...
                max_reads = max(
                    1,
                    int(kwargs["total_time"] / read_cycle_time),
                )

                xy8_block = f"""
                    {self.set_pulse_phases("x", kwargs["iq_phases"])}
                    {self.create_iq_pulses(
                        1, 1, "I_pihalf_x", 2, "Q_pihalf_x"
                    )}
                    waitWave();
