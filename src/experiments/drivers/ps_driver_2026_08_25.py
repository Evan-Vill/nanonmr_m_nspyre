...
class Pulses():
    ...
    def Power_Calibration(self, laser_time, params, read_time):
        """
        Single Rabi sequence for one microwave power.
        """
        ...
        def SingleRabi(iq_on):
            ...
            mw_iq_on_seq = [
                (iq_off1, 0),
                (self.awg_trig_time, 1),
                (iq_off2, 0)
            ]

            mw_iq_off_seq = [
                (iq_off1, 0),
                (self.awg_trig_time, 0),
                (iq_off2, 0)
            ]

            seq_on.setDigital(3, laser_seq)
            seq_on.setDigital(1, dig_clock_seq)
            seq_on.setDigital(2, mw_iq_on_seq)

            seq_off.setDigital(3, laser_seq)
            seq_off.setDigital(1, dig_clock_seq)
            seq_off.setDigital(2, mw_iq_off_seq)
    ...
    def Nitrogen_T1(self, params, read_time, cnot_ne, laser_time):
    
            #optical polarization
                #time depends on laser power
            #cnot e|n
                #t= 286 ns [inherent, based on 3.03 MHz splitting]
            #cnot n|e
            #polarize
            #t_op
                #sweep after determining cnot n|e
            #cnot e|n
            #readout (laser/digitizer)
    
            longest_time = self.convert_type(round(max(params)), float)
            read_time = self.convert_type(round(read_time), float)
            polarize_time= self.convert_type(round(laser_time), float)
            cnot_en= 286 #ns
    
            cnot_ne= int(round(cnot_ne))
    
            def SingleNitrogen_T1(t_op):
                #Creates one T1 sequence to repeatedly call throughout experiment
    
                singlet_decay= 500
                buffer= 100
    
                t_op=int(round(t_op))
                pad_time= longest_time- t_op
    
                laser_off1= self.initial_delay
                laser_on1= polarize_time
                laser_off2= singlet_decay + cnot_en + buffer + cnot_ne + buffer
                laser_on2= polarize_time
                laser_off3= singlet_decay
                laser_on3= t_op
                laser_off4= buffer + cnot_en + buffer
                laser_on4= read_time
                laser_off5= self.initial_delay + pad_time
    
                laser_seq= [(laser_off1, 0),(laser_on1, 1), (laser_off2,0), (laser_on2, 1),
                            (laser_off3,0), (laser_on3,1), (laser_off4, 0), (laser_on4,1), (laser_off5,0)]
                
                clock_off1 = laser_off1 + laser_on1 + laser_off2 + laser_on2 + laser_off3 + laser_on3 + laser_off4
                clock_off2 =  laser_on4 + laser_off5 - self.clock_time
    
                dig_clock_seq = [(clock_off1, 0), (self.clock_time, 1), (clock_off2, 0)]
    
                mw_seq = [
                    (laser_off1 + laser_on1 + singlet_decay, 0),
                    (self.awg_trig_time, 1),(cnot_en - self.awg_trig_time+ buffer, 0),
                      # wait during RF + tau
                    (buffer + cnot_ne + buffer + polarize_time + singlet_decay + t_op + buffer, 0),
                    # second MW pulse
                    (self.awg_trig_time, 1),(cnot_en - self.awg_trig_time, 0),
                    (buffer + laser_on4 + laser_off5, 0)
                ]
    
                rf_seq = [(laser_off1 + laser_on1 + singlet_decay + cnot_en + buffer, 0),
                          (self.awg_trig_time, 1),(cnot_ne - self.awg_trig_time, 0),
                          (buffer + laser_on2 + laser_off3 + laser_on3 + laser_off4 + laser_on4 + laser_off5, 0)]
    
                seq = self.Pulser.createSequence()
                seq_ref = self.Pulser.createSequence()
    
                seq.setDigital(3, laser_seq)
                seq.setDigital(2, mw_seq)
                seq.setDigital(4, rf_seq)
                seq.setDigital(1, dig_clock_seq)
    ...
    def QLE_XY8_N(self, NT1, cnot_en, cnot_ne, laser_time, params, pulse_axes, pihalf_x, pihalf_y, pi_x, pi_y, n, read_time):
            #quantum logic enhanced xy8 pulse sequence
            ...
            #total time of each iteration limited by nitrogen T1
            total_time = self.convert_type(round(NT1), float)
    
            #each iteration:
                # initialize + sensing sequence + swap gate + repeated readout
            ...
            def SingleXY8(tau):
                ...
                xy8_time = 2*pihalf_x + ((tau/2) + 4*pi_x + 4*pi_y + 7*tau + (tau/2))*n
                yy8_time = 2*pihalf_x + ((tau/2) + 8*pi_y + 7*tau + (tau/2))*n
                seq_time = xy8_time if pulse_axes == 'xy' else yy8_time
    
                # timing
                laser_off1 = self.initial_delay
    
                # XY8 + SWAP
                swap_time = self.MW_buffer_time + cnot_en + self.MW_buffer_time + cnot_ne + self.MW_buffer_time
                laser_off2 = self.singlet_decay + seq_time + swap_time
    
                clock_off1 = laser_off1 + laser_time + laser_off2
    
                # init before repetitive readout
                reinit_block = laser_time + self.singlet_decay
    
                base_time = laser_off1 + laser_time + laser_off2 + reinit_block
    
                # readout
                read_cycle = cnot_en + self.MW_buffer_time + read_time 
                remaining_time = total_time - base_time
                num_reads = max(1, int(remaining_time / read_cycle))
    
                # ---- LASER ----
                laser_seq = [
                    (laser_off1, 0),
                    (laser_time, 1),                  # init
                    (laser_off2, 0),                  # XY8 + SWAP
                    (laser_time, 1),                  # reinit
                    (self.singlet_decay, 0)
                ]
    
                # ---- DIGITIZER ----
                dig_clock_seq = [(clock_off1, 0)]
    
                # ---- MW (CNOT_en + XY8) ----
                iq_seq = []
    
                # idle until MW starts
                iq_seq += [(laser_off1 + laser_time + self.singlet_decay, 0)]
    
                # XY8 block
                iq_seq += [(self.awg_trig_time, 1), (pihalf_x - self.awg_trig_time, 0)]
                iq_seq += PiPulsesN(pulse_axes, tau, n)
    
                for _ in range(num_reads):
                    # CNOT_en (MW)
                    iq_seq.append((self.awg_trig_time, 1),
                    (cnot_en - self.awg_trig_time, 0), (self.MW_buffer_time, 0))
    
                    laser_seq.append((cnot_en, 0),(self.MW_buffer_time, 0),(read_time, 1))
                    dig_clock_seq.append((cnot_en, 0),(self.MW_buffer_time, 0),(self.clock_time, 1),(read_time - self.clock_time, 0))
    
    
                used = num_reads * read_cycle
                laser_off_end = max(0, remaining_time - used)
    
                laser_seq.append((laser_off_end, 0))
                dig_clock_seq.append((laser_off_end, 0))
                iq_seq.append((laser_off_end, 0))
                
                # ---- RF (CNOT_ne) ----
                rf_seq = [
                    (laser_off1 + laser_time + self.singlet_decay + seq_time + cnot_en + self.MW_buffer_time, 0),
                    (self.awg_trig_time, 1),
                    (cnot_ne - self.awg_trig_time, 0),
                    (remaining_time, 0), (laser_off_end,0)
                ]
    
                seq = self.Pulser.createSequence()
                seq.setDigital(3, laser_seq)
                seq.setDigital(1, dig_clock_seq)
                seq.setDigital(2, iq_seq)
                seq.setDigital(4, rf_seq)
    ...
    def QLE_Corr_Spectroscopy(self, NT1, cnot_en, cnot_ne,laser_time, pulse_axes, params, tau, pihalf_x, pihalf_y, pi_x, pi_y, n, read_time, readout_reps=10):
            ...
            n = self.convert_type(round(n), int)
            read_time = self.convert_type(round(read_time), float)
            readout_reps = self.convert_type(round(readout_reps), int)
    
            # -------- XY8 BLOCK (same structure as QLE_XY8_N) --------
            def PiPulsesN(axes, tau, N):
                if axes == 'xy':
                    # xy4_I_seq = [self.Pi('x', pi_x)[0], (tau, self.IQ0[0]), self.Pi('y', pi_y)[0], (tau, self.IQ0[0]), self.Pi('x', pi_x)[0], (tau, self.IQ0[0]), self.Pi('y', pi_y)[0]]
                    xy8_iq_seq = [(tau/2, 0), (self.awg_trig_time, 1), 
            ...
            def XY8_time(t_corr):
                return pihalf_x + (tau/2 + 0*pi_x + 4*pi_x + 4*pi_y + 7*tau + tau/2)*n + pihalf_y + t_corr + pihalf_x + (tau/2 + 0*pi_x + 4*pi_x + 4*pi_y + 7*tau + tau/2)*n + pihalf_y
            ...
            def SingleCorrSpec(t_corr):
                ...
                # XY8_time() already includes:
                # XY8 + Tcorr + XY8
                corr_spec_time = xy8_t
                ...
                num_reads = readout_reps
    
                used_time = (
                    corr_spec_time +
                    swap_time +
                    num_reads * read_cycle
                )
    
                remaining_time = max(0, NT1 - used_time)
                ...
                laser_off1 = self.initial_delay
                laser_off2 = self.singlet_decay + corr_spec_time
                clock_off1 = laser_off1 + laser_time + laser_off2
    
                laser_seq = [
                    (laser_off1, 0),
                    (laser_time, 1),      # initialization
                    (laser_off2, 0)
                ]
                ...
                # idle until XY8 starts
                iq_seq += [
                    (laser_off1 + laser_time + self.singlet_decay, 0)
                ]
    
                # first XY8
                iq_seq += [
                    (self.awg_trig_time, 1),
                    (pihalf_x - self.awg_trig_time, 0)
                ]
                ...
                # ============================================================
                # SWAP MW PULSE (CNOT_en)
                # ============================================================
    
                iq_seq += [
                    (self.MW_buffer_time, 0),
                    (self.awg_trig_time, 1),
                    (cnot_en - self.awg_trig_time, 0),
                    (self.MW_buffer_time + cnot_ne + self.MW_buffer_time + read_time + self.singlet_decay, 0)
                ]
    
                # ============================================================
                # FIRST READOUT AFTER SWAP
                # ============================================================
                ...
                dig_clock_seq += [
                    (swap_time + read_time + self.singlet_decay, 0)
                ]
    
                # ============================================================
                # REPEATED QLR READOUTS
                # ============================================================
                ...
                for _ in range(num_reads):
                    ...
                    # buffer before optical readout
                    (self.MW_buffer_time + read_time + self.singlet_decay, 0)
                ]
                    ...
                    (self.clock_time, 1),
                    (read_time - self.clock_time + self.singlet_decay, 0)
                ]
    
                # ============================================================
                # WAIT UNTIL NT1 ELAPSES
                # ============================================================
    
                laser_seq.append((remaining_time, 0))
                dig_clock_seq.append((remaining_time, 0))
                iq_seq.append((remaining_time, 0))
    ...
    def Cnot_NE_time(self, params, read_time, laser_time):
            #optical polarization, time depends on laser power
            #cnot e|n, t= 286 ns [inherent, based on 3.03 MHz splitting]
            #cnot n|e
                # "This gate, CNOT n|e, is calibrated using the pulse sequence  with T_op = 0, where the duration of the RF pulse is varied "
            #polarize
            #t_op= 0
            #cnot e|n
            #readout
            ...
            def Single_CNOT(rf_time):
                #Creates one sequence to repeatedly call throughout experiment
    
                rf_time=int(round(rf_time))
                pad_time= longest_time- rf_time
    
                laser_off1= self.initial_delay
                laser_on1= polarize_time
                laser_off2= singlet_decay + cnot_en + buffer + rf_time + buffer
                laser_on2= polarize_time
                laser_off3= singlet_decay + cnot_en + buffer
                laser_on3= read_time
                laser_off4= self.initial_delay + pad_time
    
                laser_seq= [(laser_off1, 0),(laser_on1, 1), (laser_off2,0), (laser_on2, 1),
                            (laser_off3, 0), (laser_on3,1), (laser_off4,0)]
                
                clock_off1 = laser_off1 + laser_on1 + laser_off2 + laser_on2 + laser_off3
                clock_off2 = laser_on3 + laser_off4 - self.clock_time
                ...
                mw_seq = [#wait
                    ...
                    (self.awg_trig_time, 1),(cnot_en - self.awg_trig_time, 0),
                    # wait
                    (buffer + rf_time + buffer+ laser_on2 + singlet_decay, 0),
                    # second MW pulse
                    (self.awg_trig_time, 1), (cnot_en - self.awg_trig_time, 0),
                    # wait
                    (buffer + laser_on3 + laser_off4, 0)
                ]
    
                rf_seq = [
                    (laser_off1 + laser_on1 + singlet_decay + cnot_en + buffer, 0),
                    (self.awg_trig_time, 1),(rf_time - self.awg_trig_time, 0), # swept
                    (buffer + laser_on2 + laser_off3 + laser_on3 + laser_off4, 0)
                ]
