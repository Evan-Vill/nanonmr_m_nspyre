        ...
        with QtCore.QMutexLocker(self.plot_settings.sink_mutex):
            ...
            with QtCore.QMutexLocker(self.plot_settings.mutex):
                for plot_name in self.plot_settings.series_settings:
                    ...
                    #     continue
                        ...
                        if processing == 'Append':
                            ...
                            if series == 'diff' or series == 'diff dq1' or series == 'diff dq2' or series == 'div' or series == 'div_rf' or series == 'contrast' or series == 'dark_contrast' or series == 'echo_contrast' or series == 'cd_contrast' or series == 'fft' or series == 'diff_py' or series == 'diff_ny' or series == 'diff_osc_with_pulse' or series == 'diff_osc_without_pulse':
                                ...
                                masked_data_sig = np.ma.array(
                                    stacked_data_sig, mask=np.isnan(stacked_data_sig)
                                )
                                masked_data_bg = np.ma.array(
                                    stacked_data_bg, mask=np.isnan(stacked_data_bg)
                                )
                                # average the numpy arrays
                                processed_data_sig = np.ma.average(masked_data_sig, axis=0)
                                processed_data_bg = np.ma.average(masked_data_bg, axis=0)
                            elif series == 'deer_contrast' or series == 'deer_log_contrast' or series == 'deer_diff':
                                # create a single numpy array
                                ...
                                # mask the NaN entries
                                masked_data_dark_sig = np.ma.array(
                                    stacked_data_dark_sig, mask=np.isnan(stacked_data_dark_sig)
                                )
                                masked_data_dark_bg = np.ma.array(
                                    stacked_data_dark_bg, mask=np.isnan(stacked_data_dark_bg)
                                )
                                masked_data_echo_sig = np.ma.array(
                                    stacked_data_echo_sig, mask=np.isnan(stacked_data_echo_sig)
                                )
                                masked_data_echo_bg = np.ma.array(
                                    stacked_data_echo_bg, mask=np.isnan(stacked_data_echo_bg)
                                )
                                # average the numpy arrays
                                processed_data_dark_sig = np.ma.average(masked_data_dark_sig, axis=0)
                                processed_data_dark_bg = np.ma.average(masked_data_dark_bg, axis=0)
                                processed_data_echo_sig = np.ma.average(masked_data_echo_sig, axis=0)
                                processed_data_echo_bg = np.ma.average(masked_data_echo_bg, axis=0)
                            elif series == 'diff_overall' or series == 'sum_nuclear':
                                # create a single numpy array
                                ...
                                # mask the NaN entries
                                masked_data_wpy = np.ma.array(
                                    stacked_data_wpy, mask=np.isnan(stacked_data_wpy)
                                )
                                masked_data_wny = np.ma.array(
                                    stacked_data_wny, mask=np.isnan(stacked_data_wny)
                                )
                                masked_data_nopy = np.ma.array(
                                    stacked_data_nopy, mask=np.isnan(stacked_data_nopy)
                                )
                                masked_data_nony = np.ma.array(
                                    stacked_data_nony, mask=np.isnan(stacked_data_nony)
                                )
                                # average the numpy arrays
                                processed_data_wpy = np.ma.average(masked_data_wpy, axis=0)
                                processed_data_wny = np.ma.average(masked_data_wny, axis=0)
                                processed_data_nopy = np.ma.average(masked_data_nopy, axis=0)
                                processed_data_nony = np.ma.average(masked_data_nony, axis=0)
                            elif series == 'fit':
                                processed_data_x_fit = np.ma.array(data_x_fit, mask=np.isnan(data_x_fit))
                                processed_data_y_fit = np.ma.array(data_y_fit, mask=np.isnan(data_y_fit))
                            else:
                                # create a single numpy array
                    ...
                    # print("setting data...")
                    # print("x: ", processed_data[0]) # used to debug missing plots -- typically due to incorrect units (s instead of ns for example)
                    # print("y: ", processed_data[1])
                    # update the plot
                    try:
