...
class ExpWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.cal_seq_opts = ["Rabi", "Pulsed ODMR", "T2 Echo"]
        ...
        self.readout_cal_params_defaults = [self.cal_seq_opts, 100, 100]   
        self.readout_cal_mw_params_defaults = [2.87e9, 1e-9, 100e-9]
        ...
        self.exp_dict = {"Signal vs Time": ["sigvstime_scan", self.sigvstime_params_defaults, self.sigvstime_mw_params_defaults, 'sigvstime', self.laser_params_sigvstime_defaults, self.digitizer_defaults], # ODMR MW params hidden and serves as placeholder for sig vs time experiment in GUI
                    ...
                    "Readout Calibration": ["readout_calibration_scan", self.readout_cal_params_defaults, self.readout_cal_mw_params_defaults, 'readout cal', self.laser_params_defaults, self.digitizer_defaults],
                    ...
                    "NMR Noise Spectroscopy": ["Corr_Spec_scan", self.nmr_params_defaults, self.nmr_mw_params_defaults, 'nmr', self.laser_params_defaults, self.digitizer_defaults],
    ...
    def create_params_widget(self, tag, defaults): 
        match tag:
            ...
            case 'Readout Calibration':
                params = {
                        'cal_seq': {'display_text': 'Calibration Sequence: ',
                                'widget': ComboBox(items = defaults[0])},
                        'runs': {'display_text': '# Averages per Iteration: ',
                                'widget': SpinBox(value = defaults[1], int = True, bounds=(1, None))},
    ...
    def create_mw_params_widget(self, tag, defaults, overrides=None):
        ...
        match tag:
            ...
            case 'Readout Calibration':
                params = {
                        'freq': {'display_text': 'NV/Center Frequency: ',
    ...
    def _handle_exp_message(self, msg):
        if isinstance(msg, dict):
            ...
            fit_list = [fit_value, fit_error]
            if fit_value is not None and fit_error is not None:
                self.fit_params_widget.set_fit_labels([fit_value, fit_error])
            ...
            # OLD format fallback
            queueText = msg
            percent = int(queueText[0])
            status = queueText[1]
            fit_list = queueText[2] if len(queueText) > 2 else None
            if isinstance(fit_list, list) and len(fit_list) >= 2 and all(v is not None for v in fit_list[:2]):
                self.fit_params_widget.set_fit_labels(fit_list)
        
            exc_text = queueText[3] if len(queueText) > 3 else None
    ...
    def run(self):
        ...
        #         full_dir = r"E:\Data"
            ...
            if self.daq_b1.isChecked(): # digitizer settings
                reload(nv_experiments_all_5)
                # call the function in a new process
                self.run_proc.run(
                    run_experiment,
                    exp_cls = nv_experiments_all_5.SpinMeasurements,
