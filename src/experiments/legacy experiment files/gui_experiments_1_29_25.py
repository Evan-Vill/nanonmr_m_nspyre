...
class ExpWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.odmr_params_defaults = [12, 10, 2.87e9, 100e6, 30, 1e-9, 25e-6]
        self.rabi_params_defaults = [12, 10, 0, 500e-9, 100, 2.87e9, 1e-9, self.rabi_axis_opts]#, self.rabi_device_opts]    
        self.pulsed_odmr_params_defaults = [12, 10, 2.87e9, 100e6, 30, 1e-9, 100e-9]
        self.opt_t1_params_defaults = [12, 10, 50e-9, 100e-6, 100, self.opt_t1_array_opts]
        self.mw_t1_params_defaults = [12, 10, 50e-9, 100e-6, 100, self.mw_t1_array_opts, 2.87e9, 1e-9, 20e-9, 'y']
        self.t2_params_defaults = [12, 10, 50e-9, 20e-6, 100, self.t2_array_opts, 2.87e9, 1e-9, 20e-9, 'y', self.t2_seq_opts, 1]

        self.dq_params_defaults = [12, 10, 50e-9, 100e-6, 100, self.dq_array_opts, 2.87e9, 1e-9, 20e-9, 2.87e9, 1e-9, 20e-9, 'y']

        self.deer_params_defaults = [12, 10, 350e6, 750e6, 201, 800e-9, 2.87e9, 1e-9, 20e-9, 'y', 0.2, 40e-9, self.deer_drive_opts]      
        self.deer_rabi_params_defaults = [12, 10, 3e-9, 100e-9, 100, 800e-9, 2.87e9, 1e-9, 20e-9, 'y', 560e6, 0.2]     
        self.deer_fid_params_defaults = [12, 10, 50e-9, 20e-6, 100, self.fid_array_opts, 2.87e9, 1e-9, 20e-9, 'y', 560e6, 0.2, 40e-9, 1]
        
        self.deer_fid_cd_params_defaults = [12, 10, 50e-9, 20e-6, 100, self.fid_cd_array_opts, 2.87e9, 1e-9, 20e-9, 'y', 560e6, 0.2, 40e-9, 0.1, 1]      
        
        self.deer_corr_rabi_params_defaults = [12, 10, 3e-9, 100e-9, 100, 800e-9, 1e-6, 2.87e9, 1e-9, 20e-9, 'y', 560e6, 40e-9, 0.2, 300]
        self.deer_corr_t1_params_defaults = [12, 10, 50e-9, 1e-6, 100, self.corr_t1_array_opts, 800e-9, 2.87e9, 1e-9, 20e-9, 'y', 560e6, 40e-9, 0.2]
        self.nmr_params_defaults = [12, 10, 50e-9, 100e-6, 100, 1e-6, 2.87e9, 1e-9, 20e-9, 'y', 1]

        self.casr_params_defaults = [12, 10, 10, 1e-6, 2.87e9, 1e-9, 20e-9, 1]
        
        # experiment dictionary - associates experiment function, default parameter array, dataset, 
        self.exp_dict = {"Signal vs Time": ["sigvstime_scan", self.sigvstime_params_defaults, 'sigvstime', self.laser_params_defaults, self.digitizer_defaults],
                    "CW ODMR": ["odmr_scan", self.odmr_params_defaults, 'odmr', self.laser_params_defaults, self.digitizer_defaults],
                    "Laser": [None, self.laser_params_defaults],
                    "Digitizer": [None, self.digitizer_defaults],
                    "Pulsed ODMR": ["pulsed_odmr_scan", self.pulsed_odmr_params_defaults, 'odmr', self.laser_params_defaults, self.digitizer_defaults],
                    "Rabi": ["rabi_scan", self.rabi_params_defaults, 'rabi', self.laser_params_defaults, self.digitizer_defaults],
                    "Optical T1": ["OPT_T1_scan", self.opt_t1_params_defaults, 't1', self.laser_params_defaults, self.digitizer_defaults],
                    "MW T1": ["MW_T1_scan", self.mw_t1_params_defaults, 't1', self.laser_params_defaults, self.digitizer_defaults],
                    "T2": ["T2_scan", self.t2_params_defaults, 't2', self.laser_params_defaults, self.digitizer_defaults],
                    "DQ Relaxation": ["DQ_scan", self.dq_params_defaults, 'dq', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER": ["DEER_scan", self.deer_params_defaults, 'deer', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER Rabi": ["DEER_rabi_scan", self.deer_rabi_params_defaults, 'deer rabi', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER FID": ["DEER_FID_scan", self.deer_fid_params_defaults, 'fid', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER FID Continuous Drive": ["DEER_FID_CD_scan", self.deer_fid_cd_params_defaults, 'fid cd', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER Correlation Rabi": ["DEER_corr_rabi_scan", self.deer_corr_rabi_params_defaults, 'corr rabi', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER T1": ["DEER_corr_T1_scan", self.deer_corr_t1_params_defaults, 'corr t1', self.laser_params_defaults, self.digitizer_defaults],
                    "DEER T2": ["DEER_corr_T1_scan", self.deer_corr_t1_params_defaults, 'corr t2', self.laser_params_defaults, self.digitizer_defaults],
                    "NMR: Correlation Spectroscopy": ["Corr_Spec_scan", self.nmr_params_defaults, 'nmr', self.laser_params_defaults, self.digitizer_defaults],
                    "NMR: CASR": ["CASR_scan", self.casr_params_defaults, 'nmr', self.laser_params_defaults, self.digitizer_defaults]}
        ...
        self.daq_b1.setGraphicsEffect(self.opacity_effects[7])
        ...
        self.daq_b2.setGraphicsEffect(self.opacity_effects[8])
        ...
        # self.save_proc = ProcessRunner()

        # select directory button
        self.select_dir_button = QPushButton("Select Directory")
        self.select_dir_button.setEnabled(False)
        self.select_dir_button.clicked.connect(lambda: self.select_directory())
        self.select_dir_button.setGraphicsEffect(self.opacity_effects[9])

        # selected directory display for saving
        self.chosen_dir = QLabel()
        self.chosen_dir.setGraphicsEffect(self.opacity_effects[10])
        self.chosen_dir.setStyleSheet("color: #ffa500")

        self.filename_label = QLabel("Filename: ")
        self.filename_label.setGraphicsEffect(self.opacity_effects[11])
        self.filename_label.setFixedHeight(20)

        self.filename_lineedit = QLineEdit()
        self.filename_lineedit.setGraphicsEffect(self.opacity_effects[12])
        ...
        self.fit_label.setGraphicsEffect(self.opacity_effects[12]) # TODO: update opacity effects in GUI
        ...
        self.fit_frame.setStyleSheet("background-color: #454545")
        ...
        self.fit_layout.addWidget(self.fit_label,1,2,1,1)
        ...
        self.dig_frame.setStyleSheet("background-color: #2b2b2b")
            ...
            case 'CW ODMR':    
                params = {
                        ...
                        'probe': {'display_text': 'MW Probe Time: ',
                                'widget': SpinBox(value = defaults[6], suffix = 's', siPrefix = True, bounds = (10e-9, None))}}
            case 'Laser':    
                params = {
    ...
    def save_params_clicked(self):
        ...
        self.exp_dict[self.experiments.currentText()][3] = saved_laser_params
        self.exp_dict[self.experiments.currentText()][4] = saved_dig_params
    ...
    def exp_selector(self):
        ...
        #     print("EXPERIMENT SELECTED: ", self.experiments.currentText())
            ...
            self.dig_params_widget = ParamsWidget(self.create_params_widget(' ', self.exp_dict['Digitizer'][1]), get_param_value_funs = {ComboBox: self.get_combobox_val})
...
#         return params
    ...
    def run(self):
        ...
        self.extra_kwarg_params['dataset'] = self.exp_dict[self.experiments.currentText()][2]
        ...
        if self.daq_b1.isChecked(): # digitizer settings
            reload(nv_experiments_1_20_25)
            # call the function in a new process
            self.run_proc.run(
            run_experiment,
            exp_cls = nv_experiments_1_20_25.SpinMeasurements,
    ...
    def stop(self, log: bool = True):
        ...
        # check if requested auto save 
        # self.save_kwarg_params['dataset'] = self.exp_dict[self.experiments.currentText()][2]
        # self.save_kwarg_params['filename'] = self.filename_lineedit.text()
        # self.save_kwarg_params['directory'] = self.chosen_dir.text()

        # # unpack all keyword arg parameters to send to experiment process
        # fun_kwargs = dict(**self.save_kwarg_params)
